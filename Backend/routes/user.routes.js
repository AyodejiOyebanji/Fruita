...
const { generateToken, isAuth } = require('../sec.js');
const expressAsyncHandular = require('express-async-handler');
const userRouter = express.Router();

userRouter.post(
  '/signin',
  expressAsyncHandular(async (req, res) => {
      ...
      if (result) {
        ...
        form.save((err) => {
          if (err) {
            res
              .status(501)
              .send({ message: 'Unable to SignUp, try again', status: false });
          } else {
            res.send({
